...
def _input(placeholder="", default=""):
    ...
    e.setStyleSheet(f"""
        QLineEdit {{
            ...
            border-radius:12px; font-size:13px; color:{C_TEXT};
            padding:0 16px;
        }}
...
def _combo(items: list[str]):
    ...
    c.setStyleSheet(f"""
        ...
        QComboBox QAbstractItemView {{
            ...
            border-radius:8px; selection-background-color:#FFEBEE;
...
# ─────────────────────────────────────────────────────────────────────────────
# Section card (THÔNG TIN CÁ NHÂN / THÔNG TIN LIÊN HỆ)
# ─────────────────────────────────────────────────────────────────────────────
class SectionCard(QWidget):
    def __init__(self, icon: str, title: str, parent=None):
        ...
        self._root.setContentsMargins(24, 22, 24, 24)
        self._root.setSpacing(0)

        # Header
        hdr = QHBoxLayout(); hdr.setSpacing(12)
        icon_lbl = lbl(icon, 18, 400, C_RED)
        icon_lbl.setFixedWidth(24)
        hdr.addWidget(icon_lbl)
        hdr.addWidget(lbl(title, 12, 700, C_TEXT, 1.2))
        hdr.addStretch()
    ...
    def add_row(self, widgets: list):
        """Add a row of label+field pairs."""
...
class BookingSummaryCard(QWidget):
    ...
    def __init__(self, ctx: dict, parent=None):
        ...
        root.addWidget(lbl("Tóm tắt đặt vé", 18, 800, C_TEXT))
        root.addSpacing(20)

        # Flight type
        ft_row = QHBoxLayout()
        ft_row.setSpacing(14)
        globe_badge = QLabel("🌐")
        globe_badge.setFixedSize(44, 44)
        globe_badge.setAlignment(Qt.AlignCenter)
        globe_badge.setStyleSheet(
            f"background:{C_LGRAY}; border-radius:22px; font-size:20px;"
        )
        ft_info = QVBoxLayout(); ft_info.setSpacing(3)
        ft_info.addWidget(lbl("LOẠI CHUYẾN BAY", 10, 600, C_GRAY, 1.0))
        ft_info.addWidget(lbl("QUỐC NỘI (DOMESTIC)", 13, 700, C_TEXT))
        ft_row.addWidget(globe_badge)
        ft_row.addLayout(ft_info)
        ft_row.addStretch()
        root.addLayout(ft_row)
        root.addSpacing(18)
        ...
        def _status_row(key, val, val_color=C_TEXT):
            r = QHBoxLayout()
            r.addWidget(lbl(key, 12, 500, C_GRAY))
            r.addStretch()
            r.addWidget(lbl(val, 12, 700, val_color))
            return r

        root.addLayout(_status_row("Trạng thái ghế", "ĐANG CHỜ CHỌN", C_ORANGE))
        root.addSpacing(12)
        root.addLayout(_status_row("Hành lý dự kiến", "7KG XÁCH TAY", C_TEXT))
        root.addSpacing(20)
        root.addWidget(h_sep())
        root.addSpacing(20)
...
class PassengerInfoPage(QWidget):
    proceed = Signal(dict)   # phát passenger_info dict
    go_back = Signal()

    def __init__(self, ctx: dict | None = None, account: dict | None = None, parent=None):
        super().__init__(parent)
        self.ctx = ctx or {}
        ...
        # Row: DOB + gender
        dob_col = QVBoxLayout(); dob_col.setSpacing(6)
        dob_col.addWidget(_field_label("NGÀY SINH"))
        self._dob = _input("DD/MM/YYYY")
        dob_col.addWidget(self._dob)

        gender_col = QVBoxLayout(); gender_col.setSpacing(6)
        gender_col.addWidget(_field_label("GIỚI TÍNH"))
        self._gender = _combo(["Nam", "Nữ", "Khác"])
        gender_col.addWidget(self._gender)

        row1 = QHBoxLayout(); row1.setSpacing(16)
        row1.addLayout(dob_col); row1.addLayout(gender_col)
        ...
        # Card 2: Contact info
        card2 = SectionCard("📋", "THÔNG TIN LIÊN HỆ")
        ...
        phone_col.addWidget(_field_label("SỐ ĐIỆN THOẠI"))
        self._phone = _input("+84 9xx xxx xxx")
        phone_col.addWidget(self._phone)

        row2 = QHBoxLayout(); row2.setSpacing(16)
        row2.addLayout(email_col); row2.addLayout(phone_col)
        card2._root.addLayout(row2)
        ...
        # ── Right: summary ────────────────────────────────────────────────────
        self._summary = BookingSummaryCard(self.ctx)
        self._summary.proceed.connect(self._on_proceed)
        right_w = QWidget(); right_w.setStyleSheet("background:transparent;")
        rl = QVBoxLayout(right_w); rl.setContentsMargins(0,0,0,0)
        rl.addWidget(self._summary); rl.addStretch()
...
if __name__ == "__main__":
    ...
    app.setStyle("Fusion")
    win = QMainWindow()
    win.setWindowTitle("JetJet Air — Thông tin Hành khách")
    win.resize(1380, 860)
    w = QWidget(); lay = QVBoxLayout(w); lay.setContentsMargins(0,0,0,0); lay.setSpacing(0)
    lay.addWidget(NavBar(0)); lay.addWidget(PassengerInfoPage())
